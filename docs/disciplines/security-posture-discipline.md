...
2. **Guest-IoT Isolation**: VLAN 90 must have `device_isolation` enabled and zero access to internal subnets.
3. **Implicit Deny between Hub/Spoke**: No traffic is allowed between branch locations unless specifically authorized via `Carter` identity verification.
...
3. **Allow Trusted Subnets**: Explicit access for MANAGEMENT (VLAN 10) and SERVERS (VLAN 20).
