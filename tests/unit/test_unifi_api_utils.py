...
def test_unifi_api_get_devices(mock_session):
    ...
    mock_session.get.assert_called_with(
        "https://192.168.1.1:443/proxy/network/api/s/default/stat/device", verify=False
    )
...
def test_unifi_api_update_device(mock_session):
    ...
    url = (
        "https://192.168.1.1:443/proxy/network/api/s/default/rest/device/device_id_123"
    )
    data = {"name": "New Name"}
    mock_session.put.assert_called_with(url, json=data, verify=False)
